...
class DailyOverview:
    def __init__(self, datastore_path):
        ...
        self.recipient = os.environ.get('RECIPIENTS') or [
            '14607@ramco.com', 'pb.update@ramco.com', 'xavier.selvaraj@ramco.com']
    ...
    def SendEmail(self):
        ...
        subject = 'RaiSE Daily Overview' + \
            str(datetime.now().strftime('%Y-%m-%d'))
        try:
            # sent = self.yag.send(to=self.recipient, subject=subject, cc=self.developer,
            #                      contents=self.body+self.html)
            sent = self.yag.send(to=self.recipient, subject=subject,
                                 contents=self.body+self.html)
