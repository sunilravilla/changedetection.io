        ...
        if 'notification_urls' in n_object and n_object['notification_urls']:
            ...
            if len(trigger_text):
                ...
                triggered_text = html_tools.get_triggered_text(
                    content=snapshot_contents, trigger_text=trigger_text)
                if triggered_text:
        ...
        while not self.app.config.exit.is_set():
            ...
            try:
                ...
                if uuid in list(self.datastore.data['watching'].keys()):
                    ...
                    try:
                        processor = self.datastore.data['watching'][uuid].get(
                            'processor', 'text_json_diff')

                        # @todo some way to switch by name
                        if processor == 'restock_diff':
                            update_handler = restock_diff.perform_site_check(
                                datastore=self.datastore)
                        else:
                            # Used as a default and also by some tests
                            update_handler = text_json_diff.perform_site_check(
                                datastore=self.datastore)
