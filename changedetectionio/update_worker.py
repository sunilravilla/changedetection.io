...
class update_worker(threading.Thread):
    ...
    def send_content_changed_notification(self, t, watch_uuid):
        ...
        if 'notification_urls' in n_object and n_object['notification_urls']:
            ...
            logging.info(
                ">> NO Notification sent, notification_url was empty in both watch and system")
    ...
    def send_filter_failure_notification(self, watch_uuid):
        ...
        n_object = {'notification_title': 'RaiSE - Alert - CSS/xPath filter was not present in the page',
                    'notification_body': "Your configured CSS/xPath filters of '{}' for {{watch_url}} did not appear on the page after {} attempts, did the page change layout?\n\nLink: {{base_url}}/edit/{{watch_uuid}}\n\nThanks - Your Rails Team :)\n".format(
    ...
    def send_step_failure_notification(self, watch_uuid, step_n):
        ...
        threshold = self.datastore.data['settings']['application'].get(
            'filter_failure_notification_threshold_attempts')
        n_object = {'notification_title': "RaiSE - Alert - Browser step at position {} could not be run".format(step_n+1),
    ...
    def run(self):
        ...
        update_handler = fetch_site_status.perform_site_check(
        ...
        while not self.app.config.exit.is_set():
            ...
            try:
                ...
                if uuid in list(self.datastore.data['watching'].keys()):
                    ...
                    print("> Processing UUID {} Priority {} URL {}".format(
                        uuid, queued_item_data.priority, self.datastore.data['watching'][uuid]['url']))
                    now = time.time()

                    try:
                        changed_detected, update_obj, contents = update_handler.run(
                            uuid, skip_when_checksum_same=queued_item_data.item.get('skip_when_checksum_same'))
                        # Re #342
                        ...
                        self.app.logger.error(
                            "File permission error updating", uuid, str(e))
                        ...
                        self.datastore.update_watch(uuid=uuid, update_obj={
                                                    'last_error': "Got HTML content but no text found (With {} reply code).".format(e.status_code)})
                        if e.screenshot:
                            self.datastore.save_screenshot(
                                watch_uuid=uuid, screenshot=e.screenshot)
                        process_changedetection_results = False
                        ...
                        if e.status_code == 403:
                            ...
                            err_text = "Error - Request returned a HTTP error code {}".format(
                                str(e.status_code))

                        if e.screenshot:
                            self.datastore.save_screenshot(
                                watch_uuid=uuid, screenshot=e.screenshot, as_error=True)
                        if e.xpath_data:
                            self.datastore.save_xpath_data(
                                watch_uuid=uuid, data=e.xpath_data, as_error=True)
                        if e.page_text:
                            self.datastore.save_error_text(
                                watch_uuid=uuid, contents=e.page_text)
                        ...
                        if self.datastore.data['watching'][uuid].get('filter_failure_notification_send', False):
                            c = self.datastore.data['watching'][uuid].get(
                                'consecutive_filter_failures', 5)
                            c += 1
                            # Send notification if we reached the threshold?
                            threshold = self.datastore.data['settings']['application'].get('filter_failure_notification_threshold_attempts',
                                                                                           0)
                            print(
                                "Filter for {} not found, consecutive_filter_failures: {}".format(uuid, c))
                        ...
                        err_text = "Warning, browser step at position {} could not run, target not found, check the watch, add a delay if necessary.".format(
                            e.step_n+1)
                        self.datastore.update_watch(uuid=uuid, update_obj={'last_error': err_text,
                                                                           # So that we get a trigger when the content is added again
                                                                           'previous_md5': ''})
                        ...
                        if self.datastore.data['watching'][uuid].get('filter_failure_notification_send', False):
                            ...
                            threshold = self.datastore.data['settings']['application'].get('filter_failure_notification_threshold_attempts',
                                                                                           0)
                            print(
                                "Step for {} not found, consecutive_filter_failures: {}".format(uuid, c))
                            if threshold > 0 and c >= threshold:
                                if not self.datastore.data['watching'][uuid].get('notification_muted'):
                                    self.send_step_failure_notification(
                                        watch_uuid=uuid, step_n=e.step_n)
                                c = 0
                        ...
                        self.app.logger.error(
                            "Exception reached processing watch UUID 1: %s - %s", uuid, str(e))
                    ...
                    if process_changedetection_results:
                        try:
                            ...
                            if changed_detected or not watch['last_checked']:
                                ...
                                watch.save_history_text(
                                    contents=contents, timestamp=str(round(time.time())))

                            self.datastore.update_watch(
                                uuid=uuid, update_obj=update_obj)
                            ...
                            if changed_detected:
                                ...
                                if watch.history_n >= 2:
                                    if not self.datastore.data['watching'][uuid].get('notification_muted'):
                                        self.send_content_changed_notification(
                            ...
                            self.app.logger.error(
                                "Exception reached processing watch UUID 2: %s - %s", uuid, str(e))
                            self.datastore.update_watch(
                                uuid=uuid, update_obj={'last_error': str(e)})

                    if self.datastore.data['watching'].get(uuid):
                        # Always record that we atleast tried
                        count = self.datastore.data['watching'][uuid].get(
                            'check_count', 0) + 1
                        ...
                        if update_handler.screenshot:
                            self.datastore.save_screenshot(
                                watch_uuid=uuid, screenshot=update_handler.screenshot)
                        if update_handler.xpath_data:
                            self.datastore.save_xpath_data(
                                watch_uuid=uuid, data=update_handler.xpath_data)
